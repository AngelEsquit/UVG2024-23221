...
def shell_sort(arr, ascending=True): #La primera línea define una función llamada shell_sort que toma una lista de números como entrada y un parámetro opcional "ascending" que indica si la lista debe ser ordenada de forma ascendente o descendente.
    ...
    while gap > 0: #Este bucle divide la lista en sub-listas más pequeñas y las ordena utilizando el algoritmo de ordenación por inserción.
        for i in range(gap, n): #El bucle for itera sobre los elementos de la lista y utiliza el algoritmo de ordenación por inserción para ordenar las sub-listas.
            ...
            while j >= gap and (arr[j - gap] > temp if ascending else arr[j - gap] < temp): #El bucle while se ejecuta mientras j sea mayor o igual que gap y el elemento anterior sea mayor que el elemento actual si ascending es True, o menor que el elemento actual si ascending es False.
